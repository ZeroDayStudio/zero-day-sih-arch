...
async function searchCandidates(req, res) {
  ...
  const candidates = profiles.map((profile) => {
    const skills = profile.skills
      .filter(
        (skill) =>
          !req.query.skill ||
          skill.skillId.skillNode
            .toLowerCase()
            .includes(String(req.query.skill).toLowerCase()),
      )
      .sort((a, b) => b.proficiency - a.proficiency)
      .slice(0, 5);
    const verifiedEvidence = profile.evidence
    ...
    return {
      ...
      topVerifiedSkills: skills,
      verifiedEvidence,
      profileStrength: skills.length
        ? Math.round(
  ...
  return res.json({
    candidates: candidates.map((candidate) => ({
      ...candidate,
      name: names.get(candidate.userId.toString()),
    })),
  });
}
