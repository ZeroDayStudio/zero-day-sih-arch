  ...
  return <main className="bg-paper text-ink">
    <section className="border-b border-ink/10 bg-[#edf2f1]">
      <div className="mx-auto grid min-h-[620px] max-w-7xl items-center gap-12 px-5 py-16 lg:grid-cols-[1.1fr_.9fr] lg:px-10">
        <div>
          <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="eyebrow flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-leaf" />{copy.eyebrow}</motion.p>
    ...
    <section className="mx-auto max-w-7xl px-5 py-16 lg:px-10"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow">Portal access</p><h2 className="mt-2 font-serif text-4xl tracking-[-.04em]">{copy.roles}</h2></div><p className="max-w-md text-sm leading-6 text-ink/55">A common language for competency, opportunity, and institutional accountability.</p></div><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{roles.map(({ key, href, description }, index) => { const Icon = roleIcons[key]; return <motion.div key={key} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * .06 }}><Link href={href} className="group block border border-ink/10 bg-paper p-5 transition-colors hover:border-leaf hover:bg-[#f1f6f2]"><Icon size={21} className="text-leaf" /><h3 className="mt-8 text-lg font-semibold">{copy[key]}</h3><p className="mt-2 min-h-12 text-sm leading-5 text-ink/55">{description}</p><span className="mt-6 flex items-center gap-2 text-xs font-bold text-leaf">Open portal <ArrowRight size={14} className="transition-transform group-hover:translate-x-1" /></span></Link></motion.div>; })}</div></section>
  </main>;
}
