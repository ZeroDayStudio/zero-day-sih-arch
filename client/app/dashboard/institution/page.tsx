...
import { BookOpenCheck, GraduationCap, Star, TrendingUp } from "lucide-react";
...
export default function InstitutionDashboard() {
  ...
  useEffect(() => {
    if (!user?.institutionCode) {
      setError("Your account is not linked to an institution code.");
      return;
    }
    getInstitutionReport(user.institutionCode)
      ...
      <div className="min-h-screen px-5 py-8 sm:px-8 lg:px-12">
        <header>
          ...
          <h1 className="mt-2 font-serif text-4xl tracking-[-.04em] sm:text-5xl">
            {user?.name || "Institution"} programme intelligence
          </h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-ink/55">
            Translate student evidence into curriculum action with cohort-level
            skill-gap analysis and placement reporting.
          </p>
        </header>
        {error && (
          <p
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          <Metric
            ...
            detail="Linked student accounts"
          />
          <Metric
            icon={<TrendingUp size={18} />}
            label="Profile completeness"
            value={`${report?.profileCompleteness ?? "-"}%`}
            detail="Learners with profiles"
          />
          <Metric
            icon={<BookOpenCheck size={18} />}
            label="Placement rate"
            value={`${report?.placementRate ?? "-"}%`}
            detail="Accepted applications"
          />
          <Metric
            icon={<TrendingUp size={18} />}
        ...
        <section className="mt-6 border border-ink/10 bg-paper p-5 sm:p-7">
          <p className="eyebrow">Cohort diagnostics</p>
          <h2 className="mt-2 text-xl font-semibold">
            Learners by AYUSH discipline
          </h2>
          <div className="mt-7 space-y-5">
            {Object.entries(report?.disciplineCoverage || {}).map(
              ([discipline, count]) => (
                <div key={discipline}>
                  <div className="flex justify-between gap-4 text-sm">
                    <span className="font-semibold">{discipline}</span>
                    <span className="text-ink/50">{count} learners</span>
                  </div>
                  <div className="mt-2 h-3 bg-ink/10">
                    <div
                      className="h-full bg-saffron"
                      style={{
                        width: `${report?.studentCount ? Math.min(100, (count / report.studentCount) * 100) : 0}%`,
                      }}
                    />
                  </div>
                </div>
            ...
            {report && !Object.keys(report.disciplineCoverage).length && (
              <p className="text-sm text-ink/50">
                No discipline data is available yet.
              </p>
            )}
            {!report && !error && (
              <p className="text-sm text-ink/50">
        ...
        {Boolean(report?.curriculumFeedback?.length) && (
          <section className="mt-6 border border-ink/10 bg-paper p-5 sm:p-7">
            <p className="eyebrow">Curriculum feedback loop</p>
            <h2 className="mt-2 text-xl font-semibold">
              Learner reflections from completed pathways
            </h2>
            <div className="mt-4 space-y-2">
              {report?.curriculumFeedback.map((item, index) => (
                <p
                  key={`${item}-${index}`}
                  className="border-l-2 border-saffron pl-3 text-sm leading-6 text-ink/65"
                >
                  {item}
                </p>
              ))}
