...
export function GlobalNav() {
  ...
  const links = user ? [["/", copy.home], ...roleNavigation(user.role, copy).map((item) => [item.href, item.label])] : [["/", copy.home], ["/match", copy.opportunities]];
    ...
    <header className="sticky top-0 z-50 border-b border-ink/10 bg-paper/95 backdrop-blur" role="banner">
      <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-4 px-5 lg:px-10">
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? <div className="hidden items-center gap-2 sm:flex"><Link href="/settings" aria-label={copy.settings} className="flex items-center gap-2 rounded-lg border border-ink/15 px-3 py-2 text-left hover:bg-ink/5"><span className="grid h-7 w-7 place-items-center rounded-full bg-moss text-xs font-bold text-ink">{user.name.charAt(0).toUpperCase()}</span><span><strong className="block max-w-28 truncate text-xs">{user.name}</strong><small className="block text-[10px] text-ink/50">{roleLabel(user.role, { student: copy.roleStudent, institution: copy.roleInstitution, employer: copy.roleEmployer, mentor: copy.roleMentor, admin: copy.roleAdmin })}</small></span></Link><Link href="/settings" aria-label={copy.settings} className="rounded-lg border border-ink/15 p-2 text-ink/60 hover:bg-ink/5"><Settings2 size={16} /></Link><button type="button" onClick={signOut} className="rounded-lg border border-ink/15 px-3 py-2 text-xs font-semibold text-ink/60 hover:bg-ink/5">{copy.signOut}</button></div> : <Link href="/login" className="hidden rounded-lg bg-ink px-4 py-2.5 text-sm font-semibold text-paper hover:bg-leaf sm:block">{copy.signIn}</Link>}
      ...
      {mobileOpen && <nav className="border-t border-ink/10 bg-paper px-5 py-3 md:hidden" aria-label="Mobile navigation">
        ...
        {user ? <><Link href={workspace} onClick={() => setMobileOpen(false)} className="block px-2 py-3 text-sm font-semibold text-leaf">{user.name}</Link><button type="button" onClick={() => { signOut(); setMobileOpen(false); }} className="block w-full px-2 py-3 text-left text-sm text-ink/55">{copy.signOut}</button></> : <Link href="/login" onClick={() => setMobileOpen(false)} className="block px-2 py-3 text-sm font-semibold text-leaf">{copy.signIn}</Link>}
      </nav>}
    </header>
  </>;
}
